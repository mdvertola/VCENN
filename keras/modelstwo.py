...
class JointEmbeddingModel:
    ...
    def build(self):
        ...
        git_methname_pool = Concatenate(name='concat_git_methname_lstms')([maxpool(git_methname_f_dropout), maxpool(git_methname_b_dropout)])
        git_activation = Activation('tanh',name='active_git_methname')
        git_methname_repr = activation(git_methname_pool)
       ...
       '''
        ...
        dropout = Dropout(0.25,name='dropout_stack_methname_rnn')
        stack_methname_f_dropout = dropout(stack_methname_f_rnn)
        stack_methname_b_dropout = dropout(stack_methname_b_rnn)
        #3.maxpooling
        ...
        dropout = Dropout(0.25,name='dropout_apiseq_embed')
        stack_apiseq_dropout = dropout(stack_apiseq_embedding)
        #2.rnn
        f_rnn = LSTM(self.model_params.get('n_lstm_dims', 100), return_sequences=True, recurrent_dropout=0.2,
        ...
        apiseq_b_rnn = b_rnn(apiseq_dropout)
        dropout = Dropout(0.25,name='dropout_apiseq_rnn')
        stack_apiseq_f_dropout = dropout(apiseq_f_rnn)
        stack_apiseq_b_dropout = dropout(apiseq_b_rnn)
        #3.maxpooling
        maxpool = Lambda(lambda x: K.max(x, axis=1, keepdims=False), output_shape=lambda x: (x[0], x[2]),name='maxpool_stack_apiseq')
        stack_apiseq_pool = Concatenate(name='stack_concat_apiseq_lstms')([maxpool(stack_apiseq_f_dropout), maxpool(stack_apiseq_b_dropout)])
        ...
        '''
        good_sim = sim_model([self.git_methname,self.git_apiseq,self.git_tokens,self.stack_methname_good,self.stack_apiseq_good,self.stack_tokens_good])# similarity of good output
        bad_sim = sim_model([self.git_methname,self.git_apiseq,self.git_tokens,self.stack_methname_bad,self.stack_apiseq_bad,self.stack_tokens_bad])#similarity of bad output
        ...
        self._training_model=Model(inputs=[self.git_methname,self.git_apiseq,self.git_tokens,self.stack_methname_good,self.stack_methname_bad,self.stack_apiseq_good,self.stack_apiseq_bad,self.stack_tokens_good,self.stack_tokens_bad],
